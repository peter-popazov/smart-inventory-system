...
public interface StockMovementRepository extends JpaRepository<StockMovements, Integer> {
    ...
    @Query("SELECT sm FROM stock_movements sm WHERE sm.product.productId = :productId ORDER BY sm.createdAt DESC")
    List<StockMovements> findByProductId(Integer productId);

    @Query("SELECT sm FROM stock_movements sm WHERE sm.product.userId = :userId ORDER BY sm.createdAt DESC")
